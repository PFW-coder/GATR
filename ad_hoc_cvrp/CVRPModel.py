...
class CVRPModel(nn.Module):
    ...
    def __init__(self, env_params, **model_params):
        ...
        self.decoder = CVRP_Decoder(self.env_params, **model_params)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
        ...
        elif state.selected_count == 1 and not self.model_params['random_choice'] and self.model_params['eval_type'] == 'argmax':  # Second Move, POMO
        ...
        elif state.selected_count == 1 and self.model_params['random_choice'] and self.model_params['eval_type'] == 'argmax':
            ...
            probs = self.decoder(encoded_last_node, state.load, state.used_time, state.agent_capacity,
            ...
            probs = self.decoder(encoded_last_node, state.load, state.used_time, state.agent_capacity, state.agent_speed, ninf_mask=state.ninf_mask)
...
class CVRP_Decoder(nn.Module):
    def __init__(self, env_params, **model_params):
        ...
        self.context_linear = nn.Linear(embedding_dim+4, embedding_dim, bias=False)
    ...
    def forward(self, encoded_last_node, load, used_time, agent_capacity, agent_speed, ninf_mask):
        ...
        #######################################################
        input_cat = torch.cat((encoded_last_node, load[:, :, :, None], used_time[:, :, :, None], agent_capacity[:, None, :, None].repeat(1, self.pomo_size, 1, 1), agent_speed[:, None, :, None].repeat(1, self.pomo_size, 1, 1)), dim=3)
        # shape = (batch, pomo, agent, EMBEDDING_DIM+4)
